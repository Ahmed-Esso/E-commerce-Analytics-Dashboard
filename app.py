...
def load_data():
    try:
        # Try to load from relative path first, then absolute
        try:
        ...
        st.error("⚠️ Data file not found! Please ensure cleaned_ecommerce_data.csv exists.")
        return None
...
# =============================================================================
    ...
    with tab1:
        if 'month_date' in filtered_df.columns and 'marketing_channel' in filtered_df.columns:
            st.subheader("Monthly Revenue Trends by Channel")
            monthly_channel = filtered_df.groupby(['month_date', 'marketing_channel']).agg({
                'net_revenue': 'sum'
            }).reset_index()
            
            fig = px.line(monthly_channel, x='month_date', y='net_revenue', 
                         color='marketing_channel', markers=True)
            fig.update_layout(
    ...
    with tab2:
        if 'marketing_channel' in df.columns:
            ...
            channel_revenue = filtered_df.groupby('marketing_channel')['net_revenue'].sum().reset_index()
            
            fig = px.bar(channel_revenue, x='marketing_channel', y='net_revenue',
                        color='net_revenue', color_continuous_scale=['#3647F5', '#D9D9D9', '#FF9F0D'])
            fig.update_layout(
                plot_bgcolor='rgba(0,0,0,0)',
                paper_bgcolor='rgba(0,0,0,0)',
                font_color='#f5f5f5',
    ...
    with tab3:
        if 'category' in filtered_df.columns:
            ...
            cat_revenue = filtered_df.groupby('category')['net_revenue'].sum().reset_index()
            
            fig = px.pie(cat_revenue, values='net_revenue', names='category')
            fig.update_layout(
                plot_bgcolor='rgba(0,0,0,0)',
                paper_bgcolor='rgba(0,0,0,0)',
                font_color='#f5f5f5',
...
# =============================================================================
    ...
    st.markdown("""
        <div style='text-align: center; padding: 2rem 0;'>
            <h1 style='font-size: 3rem;'>📊 E-commerce Analytics Pro</h1>
            <p style='font-size: 1.3rem; color: #00d9ff;'>
    ...
    st.markdown("""
        This is a comprehensive e-commerce analytics dashboard built with:
